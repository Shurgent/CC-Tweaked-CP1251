...
public final class LuaValues
{
    ...
    static
    {
        for( int i = 0; i < 128; i++ ) CP1251[i] = (char) i;
     ...
     */
    @Nonnull
    public static byte[] encodeBytes( @Nonnull String string )
    {
        byte[] bytes = new byte[string.length()];
        for( int i = 0; i < bytes.length; i++ ) bytes[i] = (byte) encodeChar( string.charAt( i ) );
        return bytes;
    }
     ...
     */
    public static int tryEncodeChar( char c )
    {
        if( c < 128 ) return c;
        if( c >= '\u0410' && c <= '\u042f' ) return 0xc0 + c - '\u0410';
        if( c >= '\u0430' && c <= '\u044f' ) return 0xe0 + c - '\u0430';
    ...
    public static String decode( byte[] bytes, int offset, int length )
    {
        ...
        for( int i = 0; i < length; i++ ) chars[i] = decodeByte( bytes[offset + i] );
     ...
     */
    @Nonnull
    public static String decode( @Nonnull ByteBuffer bytes )
    {
        ByteBuffer copy = bytes.slice();
        char[] chars = new char[copy.remaining()];
        for( int i = 0; i < chars.length; i++ ) chars[i] = decodeByte( copy.get() );
        return new String( chars );
    }
